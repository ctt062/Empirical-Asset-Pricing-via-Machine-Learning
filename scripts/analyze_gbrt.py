"""
Detailed GBRT (Gradient Boosted Regression Trees) Model Analysis and Visualization.

This script:
1. Loads GBRT predictions
2. Calculates detailed performance metrics (EW and VW)
3. Creates comprehensive visualizations
4. Analyzes feature importance from GBRT model
5. Saves all results to results/figures/gbrt/
...
FIGURES_DIR = RESULTS_DIR / 'figures' / 'gbrt'
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

def load_predictions():
    """Load GBRT predictions and test data."""
    logger.info("Loading GBRT predictions...")
    
    # Load predictions
    pred_df = pd.read_parquet(RESULTS_DIR / 'predictions' / 'gbrt_predictions.parquet')
    
    # Standardize column names (GBRT uses y_true and y_pred)
    if 'y_true' in pred_df.columns:
        pred_df = pred_df.rename(columns={'y_true': 'actual', 'y_pred': 'prediction'})
    
    # Reset index to get permno and date as columns
...
def plot_performance_comparison(ew_metrics, vw_metrics):
    ...
    fig.suptitle('GBRT (Gradient Boosted Trees): Equal-Weighted vs Value-Weighted Performance', 
...
def plot_portfolio_value_overtime(ew_metrics, vw_metrics):
    ...
    fig.suptitle('GBRT: Portfolio Value Over Time', fontsize=16, fontweight='bold')
    ...
    ax = axes[0]
    ew_cum_returns.plot(ax=ax, linewidth=2, color='blue', label='GBRT EW')
    ...
    ax = axes[1]
    vw_cum_returns.plot(ax=ax, linewidth=2, color='red', label='GBRT VW')
...
def plot_feature_importance(df):
    ...
    ax.set_title('GBRT: Top 20 Feature Importance\n(Correlation with Actual Returns)', 
...
def plot_tree_depth_analysis(df):
    """Analyze prediction distribution and non-linearity captured by GBRT."""
    
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('GBRT: Non-Linear Pattern Analysis', fontsize=16, fontweight='bold')
    ...
    ax.grid(True, alpha=0.3)
    ax.axvline(df['prediction'].mean(), color='red', linestyle='--', linewidth=2, label=f'Mean: {df["prediction"].mean():.4f}')
    ax.legend()
    ...
    # Add regression line
    z = np.polyfit(sample['prediction'], sample['actual'], 1)
    p = np.poly1d(z)
    x_line = np.linspace(sample['prediction'].min(), sample['prediction'].max(), 100)
    ax.plot(x_line, p(x_line), "r--", linewidth=2, label='Linear fit')
    ax.legend()
    
    # 3. Prediction deciles vs Actual returns
    ax = axes[1, 0]
    df['pred_decile'] = pd.qcut(df['prediction'], 10, labels=False, duplicates='drop')
    decile_returns = df.groupby('pred_decile')['actual'].mean() * 100  # Convert to percentage
    decile_returns.plot(kind='bar', ax=ax, color='blue', alpha=0.7)
    ax.set_title('Average Actual Return by Prediction Decile', fontsize=12, fontweight='bold')
    ...
    # Add values on bars
    for i, v in enumerate(decile_returns):
        ax.text(i, v, f'{v:.2f}', ha='center', va='bottom' if v > 0 else 'top', fontsize=9)
    
    # 4. Residuals distribution
    ax = axes[1, 1]
    df['residual'] = df['actual'] - df['prediction']
    df['residual'].hist(ax=ax, bins=50, color='orange', alpha=0.7, edgecolor='black')
    ax.set_title('Prediction Residuals Distribution', fontsize=12, fontweight='bold')
    ...
    # Add stats
    stats_text = f'Mean: {df["residual"].mean():.4f}\nStd: {df["residual"].std():.4f}'
    ax.text(0.05, 0.95, stats_text, transform=ax.transAxes, fontsize=10, va='top',
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'nonlinear_analysis.png', dpi=300, bbox_inches='tight')
    logger.info(f"Saved: {FIGURES_DIR / 'nonlinear_analysis.png'}")
    plt.close()
...
def plot_monthly_returns(ew_metrics, vw_metrics):
    ...
    fig.suptitle('GBRT: Monthly Return Distributions', fontsize=16, fontweight='bold')
    
    # Equal-weighted
    ax = axes[0]
    ew_metrics['returns'].hist(ax=ax, bins=30, color='blue', alpha=0.7, edgecolor='black')
    ax.axvline(ew_metrics['returns'].mean(), color='red', linestyle='--', linewidth=2, 
               label=f'Mean: {ew_metrics["returns"].mean()*100:.2f}%')
    ...
    ax.axvline(vw_metrics['returns'].mean(), color='blue', linestyle='--', linewidth=2, 
               label=f'Mean: {vw_metrics["returns"].mean()*100:.2f}%')
...
def main():
    ...
    logger.info("="*80)
    logger.info("GBRT MODEL - DETAILED ANALYSIS")
    logger.info("="*80)
    ...
    plot_tree_depth_analysis(df)
    
    # Save summary
    summary_df = save_summary_statistics(ew_metrics, vw_metrics)
    
    logger.info("="*80)
    logger.info("GBRT PERFORMANCE SUMMARY")
    logger.info("="*80)
